  ...
  return (
    <section aria-labelledby="quick-links-title" className="flex min-w-0 flex-col lg:col-span-1">
      ...
      <ul className="mt-4 flex flex-col gap-3">
        {quickLinks.map(({ title, description, href, icon: Icon }, index) => (
          <li key={title} className="flex h-[116px]">
            <a href={href} data-quick-link={index + 1} className="quick-link group flex h-full w-full items-center gap-3 rounded-xl border border-border bg-card p-3.5 transition">
