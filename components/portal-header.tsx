    ...
    <header className="sticky top-0 z-40 border-b border-border bg-card/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between px-5 lg:px-8">
        ...
        <nav aria-label="주요 메뉴" className="hidden items-center gap-7 md:flex">
          {navItems.map((item, index) => (
            <a key={item} href="#workspace" className={index === 0 ? "border-b-2 border-purple-primary py-5 text-sm font-bold text-blue-dark" : "py-5 text-sm text-text-secondary transition-colors hover:text-blue-primary"}>{item}</a>
          ))}
        </nav>
      ...
      {open && (
        <nav aria-label="모바일 메뉴" className="flex flex-col gap-1 border-t bg-card p-4 md:hidden">
          {navItems.map((item) => <a key={item} href="#workspace" onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 text-sm font-medium text-text-secondary hover:bg-purple-light hover:text-purple-primary">{item}</a>)}
