...
import type { NoticeFilters } from "@/components/notice/notice-types"
import { DateRangeField } from "@/components/shared/date-range-field"
import { SearchActions } from "@/components/shared/search-actions"

type NoticeSearchPanelProps = {
  filters: NoticeFilters
  isLoading: boolean
  onChange: (field: keyof NoticeFilters, value: string) => void
  onReset: () => void
  onSearch: () => void
}
...
export function NoticeSearchPanel({
  ...
  const changeFilter = (field: keyof NoticeFilters, value: string) => {
    setDateError("")
  ...
  return (
    <form
      className="contract-search-panel contract-search-panel--contract notice-search-panel"
      aria-label="알림글 검색 조건"
      autoComplete="off"
      onSubmit={(event) => {
    ...
    >
      ...
      {isDetailOpen && (
        <div id="notice-detail-search" className="contract-detail-panel">
          <div className="contract-detail-grid notice-detail-grid">
      ...
      <div className="contract-search-controls">
        <button
          ...
          aria-controls="notice-detail-search"
